...
use ranim::{
    ...
    core::{
        ...
        timeline::{ItemDynTimelines, ItemTimeline},
    },
...
impl SingleTimeline {
    pub fn update_from(&mut self, scene: &RanimScene, timeline: &ItemDynTimelines) {
...
impl VisualTimeline {
    ...
    pub fn update_inner_transform_from(
        ...
        transform_anim_modifier: impl Fn(AnimationSpan<Group<VItem>>) -> AnimationSpan<Group<VItem>>,
    ) -> ItemId<Self> {
        let prev = self.clone();
        let new = self.with(|x| x.update_inner_from(src_scene));

        let id = r.map(item_id, Group::<VItem>::from);
        r.timeline_mut(&id)
            .play_with(|x| transform_anim_modifier(x.transform_to(new.clone().into())));
        r.map(id, |_| new)
    }
...
impl From<VisualTimeline> for Group<VItem> {
...
impl Extract for VisualTimeline {
    ...
    fn extract(&self) -> Vec<Self::Target> {
        Group::<VItem>::from(self.clone()).extract()
    }
}
