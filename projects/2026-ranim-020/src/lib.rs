...
use crate::{code_structure::p4_hello_ranim_square_anim, visual_timeline::VisualTimeline};
...
pub fn test_alpha(r: &mut RanimScene) {
    ...
    let r_circles = r.insert(circles);
}
...
pub fn test(r: &mut RanimScene) {
    ...
    let a = Square::new(1.0);
    let b = Square::new(1.0).with(|x| {
        x.shift(DVec3::Y);
    });
    let r_a = r.insert(a.clone());
    let r_b = r.insert(b.clone());
    r.timeline_mut(r_a)
        .forward(2.0)
        .forward(3.0)
        .play(a.clone().fade_in());
...
pub fn code_structure(r: &mut RanimScene) {
    ...
    let _r_cam = r.insert(CameraFrame::default());
    // let mut vis_timeline = VisualTimeline::new().with_height(2.0).with_width(2.0);
    // let r_vis_timeline = r.insert(vis_timeline.clone());

    // Chained API
    /* 这，是一个 ranim 动画场景的基础代码结构。很简单，就是一个函数。*/
    let code = Vec::<VItem>::from(codes[0].clone());
    let r_code = r.insert_with(|t| {
        t.play(code.clone().lagged(0.01, |x| x.write()).with_duration(4.0))
            .forward(5.0);
    });

    /* 如果我想要为它添加视频输出，只需要添加一个 `#[output]` 宏。 */
    let mut code = codes[0].clone();
    r.timeline_mut(r_code)
        .play(code.transform_to(codes[1].clone()).with_rate_func(smooth))
        .forward(3.0) // 当然，可以添加任意数量的输出，并带有不同的参数
        .play(code.transform_to(codes[2].clone()).with_rate_func(smooth))
        .forward(2.0)
        .play(code.transform_to(codes[1].clone()).with_rate_func(smooth));
...
let r_square = r.insert(square);"#,
    ...
    r.timeline_mut(r_code)
        ...
        .play(code.transform_to(codes[3].clone()).with_rate_func(smooth))
        .forward(2.0) // 用它来在时间线上编码动画
        .play(code.transform_to(codes[4].clone()).with_rate_func(smooth));
    ...
    //     .forward(2.0);
    // vis_timeline
    //     .update_inner_from(&RanimScene::new().with(p4_hello_ranim_square_anim::hello_ranim));

    // r.timeline_mut(r_vis_timeline).play(
    //     Vec::<VItem>::from(vis_timeline.clone())
    ...
    r.timeline_mut(r_code)
        ...
        .play(code.transform_to(codes[5].clone()).with_rate_func(smooth)) // 插入一个 `TimeMark`
        .forward(5.0) // 值得一提的是，Ranim 中的绝大部分 API 都是支持链式调用的，
        .play(code.transform_to(codes[6].clone()).with_rate_func(smooth)) // 所以这些代码
        .forward(2.0) // 你可以这样写
        .play(code.transform_to(codes[7].clone()).with_rate_func(smooth))
        .forward(4.0) // 除此之外，为了支持将这种声明、修改、使用的代码写成一条语句
        .play(code.transform_to(code_tmp).with_rate_func(smooth))
    ...
    let last_time = 3.0;
    let _r_code_with_trait = r.insert_with(|t| {
        let text = TypstText::new_multiline_code(CODE_TRAIT_WITH, Some("rust")).with(|x| {
            x.scale_to_min(&[
        ...
        t.forward_to(time)
            .play(text.clone().fade_in())
            .forward(last_time)
            .play(text.clone().fade_out());
    });

    r.timeline_mut(r_code)
        .play(code.clone().fade_out())
        .forward(last_time)
        .play(code.fade_in());
    ...
    r.timeline_mut(r_code) // 所以你也可以这样写代码
        .play(code.transform_to(codes[7].clone()).with_rate_func(smooth))
        .forward(0.5)
        .play(code.transform_to(codes[8].clone()).with_rate_func(smooth))
        .forward(0.5)
        .play(code.transform_to(codes[9].clone()).with_rate_func(smooth))
        .forward(2.0);
}
